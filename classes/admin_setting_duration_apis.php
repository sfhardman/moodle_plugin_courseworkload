                ...
                if (!property_exists($item, $property)) {
                    return str_replace(
                        ...
                        str_replace('<property>', $property,
                          get_string('config_value_item_missing_property', 'report_courseworkload'))
                    );
                }
            }
            $possibleunits = html_processor::get_possible_duration_units();
            if (!in_array($item->duration_units, $possibleunits, true)) {
                return str_replace('<index>', $index,
                    str_replace('<value>', $item->duration_units,
                        str_replace('<possible_values>', join(", ", $possibleunits),
